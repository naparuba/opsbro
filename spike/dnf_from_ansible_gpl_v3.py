...
class DnfModule(object):
    ...
    def __init__(self, package_name):
        ...
        self.install_weak_deps = 'yes'
        self.bugfix = False
        self.security = False
        self.names = [package_name]
    ...
    def _mark_package_install(self, pkg_spec, upgrade=False):
        ...
        print('_mark_package_install:: %s => is installed=%s   is_newer_version_installed:%s' % (pkg_spec, is_installed, is_newer_version_installed))
        try:
            print('  allow_downgrade: %s' % self.allow_downgrade)
            if self.allow_downgrade:
                ...
                if upgrade:
                    if is_installed:
                        ...
                        print('update: INSTALLING BACKAGE: %s' % pkg_spec)
                        self.base.install(pkg_spec)
                else:
                    print('not update: INSTALLING BACKAGE: %s' % pkg_spec)
                    r = self.base.install(pkg_spec)
                    print(self.base.install)
                    
                    print('not update: INSTALLING BACKAGE: %s   DONE=%s' % (pkg_spec, r))
            else:
                if upgrade:
                    if is_installed:
                        self.base.upgrade(pkg_spec)
            ...
            print('FINISH: %s' % 'Installed: {0}'.format(pkg_spec))
    ...
    def _parse_spec_group_file(self):
        ...
        print('GIVE BACK: %s %s %s %s' % (pkg_specs, grp_specs, module_specs, filenames))
    ...
    def ensure(self):
        ...
        if self.names == ['*'] and self.state == 'latest':
            ...
            if self.state in ['installed', 'present']:
                ...
                print('MODULE SPECS: %s' % module_specs)
                # Install modules
                ...
                # Install packages.
                print('Installing packages: %s' % pkg_specs)
                if self.update_only:
                    print('UPDATE ONLY')
                    ...
                    for pkg_spec in pkg_specs:
                        print('INSTALL package: %s' % pkg_spec)
        ...
        print('ensure: allow_erasing: %s' % allow_erasing)
        try:
...
def main():
    ...
    module_implementation = DnfModule(package_name='python3-devel')
